#!/usr/bin/env python3
"""
变体 2: AR CoT+Answer（显式思维链）
====================================
带显式 CoT 推理的 SFT，推理文本在轨迹预测之前。
对应 OneVL: sft_distributed_qwen3vl_cot_answer_bs64.sh

Prompt 中包含 CoT 文本作为回答的一部分，无隐式 token。
预期结果: ADE ~6.8m（当前设置下最优），训练 ~84 分钟 (L4)。
"""
...
def build_cot_inputs(sample, processor, tokenizer, device='cuda'):
    """构建带有 CoT 文本前缀的训练输入。"""
    ...
    # Prepend CoT reasoning
    cot_prefix = "The vehicle should maintain safe trajectory based on the current driving conditions. "
    ...
    sys_text_enc = tokenizer.encode('You are a helpful assistant.', add_special_tokens=False)
    ...
    sys_part = [im_start] + sys_role + nl + sys_text_enc + [im_end] + nl
    input_ids += sys_part
    ...
    ast_prefix_ids = [im_start] + ast_role + nl
    input_ids += ast_prefix_ids
    labels += [-100] * len(ast_prefix_ids)

    # Full answer with CoT (loss on everything)
...
def main():
    ...
    log(f"{'='*60}", LOG_FILE)
    log(f"  Variant 2: AR CoT+Answer (explicit reasoning)", LOG_FILE)
    log(f"{'='*60}", LOG_FILE)
    ...
    log(f"\nTraining {TOTAL_EPOCHS} epochs with CoT prefix...", LOG_FILE)
    ...
    for epoch in range(TOTAL_EPOCHS):
        ...
        for idx, sample in enumerate(train_samples):
            try:
                batch = build_cot_inputs(sample, processor, tokenizer)
                outputs = model(
